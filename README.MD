...
| 角色 | 硬件 | 用途 |
| --- | --- | --- |
| 室外门口机 | DK01 开发套件；PIR 功能需要另购 AM312 | 门铃触发、摄像头和麦克风采集、PIR 抓拍、LR2021 发送。 |
| 室内显示网关 | DK01 开发套件 | 响铃、显示门口画面、监听、双向通话、访客记录和设置。 |

可选 AM312 PIR 模块应在断电状态下接线：

| AM312 引脚 | 连接到 DK01 | 说明 |
| --- | --- | --- |
| `VCC` | `3V3` | 建议使用 3.3 V 供电。 |
| `OUT` | `GPIO12` | 高电平触发，固件配置输入下拉。 |
| `GND` | `GND` | 传感器与开发板必须共地。 |

GPIO12 在显示网关角色中同时用于触摸中断，因此 PIR 只应连接和启用于门口机。可参考 [AM312 购买入口](https://e.tb.cn/h.8QmQiC5bW3w6Jr4?tk=MG18TXmjFKS)；商品信息以销售页面为准。
...
PIR 和低功耗状态只有在与门口机的配置交互成功后才更新，并保存在设备 NVS 中。音量保存在网关 NVS 中。
