...
#define BSP_I2C0_FREQ_HZ            (100 * 1000)   /* Conservative rate for bus margin */
...
#define BSP_LCD_TE_GPIO             GPIO_NUM_NC   /* TE is not connected */
...
/* ---------- User buttons ------------------------------------------------- */
/*
 * RST  -> CHIP_PU (reset, hardwired)
 * BOOT -> GPIO0 (strap/download button)
 * K1, K2, K4 and K5 share a resistor-divider ADC ladder on GPIO5:
 *     K1 (3.3 k) -> ~0.82 V
 *     K2 (10 k)  -> ~1.65 V
 *     K4 (27 k)  -> ~2.41 V
 *     K5 (5.1 k) -> ~1.11 V
 * K3 is the separate hardware power key and is not scanned by this BSP.
